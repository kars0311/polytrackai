...
class RacingAI:
    ...
    def choose_action(self, state, time_elapsed, trials=0):
            ...
            try:
                ...
                if weights.sum() > 0:
                    ...
                    if chosen_sequence:
                        ...
                        # Fallback to exploration/exploitation
                        action = self._select_from_q_tables(state, sequence_state)
                else:
                    # Fallback to exploration/exploitation
                    action = self._select_from_q_tables(state, sequence_state)
        ...
        # Strategy 3: Use Q-learning (exploration/exploitation)
        else:
            action = self._select_from_q_tables(state, sequence_state)
    ...
    def _select_from_q_tables(self, state, sequence_state):
        """Helper method to select action using exploration/exploitation"""
        ...
        if np.random.random() < self.epsilon:
            ...
            if np.random.random() < 0.7:
                ...
                # Bias toward actions that worked well in similar situations
                similar_sequences = [
        ...
        # Exploit: Choose best action
            ...
            # Try sequence Q-table first
            q_values = self.sequence_q_table[sequence_state]

            # Fall back to standard Q-table if no sequence data
            if not q_values:
...
if __name__ == "__main__":
    ...
    run_multi_agent(num_agents, sequence_length, False)